...
<style>
    ...
    .title {{ fill: {COLORS['fg']}; font-size: 20px; font-weight: 700; font-family: 'Inter', sans-serif; }}
    .username {{ fill: {COLORS['date']}; font-size: 13px; font-weight: 500; font-family: 'Inter', sans-serif; }}
    .stat-label {{ fill: {COLORS['date']}; font-size: 11px; font-weight: 500; font-family: 'Inter', sans-serif; text-transform: uppercase; letter-spacing: 0.5px; }}
    .stat-value {{ fill: {COLORS['title']}; font-size: 28px; font-weight: 700; font-family: 'Inter', sans-serif; }}
    .footer {{ fill: {COLORS['date']}; font-size: 10px; font-family: 'Inter', sans-serif; }}
    .icon-circle {{ fill: url(#grad1); opacity: 0.1; }}
    .divider {{ stroke: {COLORS['border']}; stroke-width: 1; opacity: 0.5; }}

    .repo-name {{ fill: {COLORS['title']}; font-size: 15px; font-weight: 600; font-family: 'Inter', sans-serif; }}
    ...
    .repo-stat {{ fill: {COLORS['date']}; font-size: 11px; font-family: 'Inter', sans-serif; }}

    .subtitle {{ fill: {COLORS['date']}; font-size: 12px; font-weight: 500; font-family: 'Inter', sans-serif; }}
    .day-label {{ fill: {COLORS['date']}; font-size: 10px; font-weight: 600; font-family: 'Inter', sans-serif; }}
    .count {{ fill: {COLORS['title']}; font-size: 11px; font-weight: 700; font-family: 'Inter', sans-serif; }}
    .baseline {{ stroke: {COLORS['border']}; stroke-width: 1; opacity: 0.3; stroke-dasharray: 4,4; }}

    .emoji {{ font-size: 18px; }}
    .time-count {{ fill: {COLORS['title']}; font-size: 14px; font-weight: 700; font-family: 'Inter', sans-serif; }}
    .time-label {{ fill: {COLORS['date']}; font-size: 9px; font-family: 'Inter', sans-serif; }}
</style>
"""
...
overview_body = f"""
  ...
  <circle class="icon-circle" cx="440" cy="35" r="55"/>
  <circle class="icon-circle" cx="40" cy="145" r="35"/>

  <text x="20" y="34" class="title">📊 GitHub Stats</text>
  <text x="20" y="52" class="username">@{USERNAME}</text>

  <line x1="165" y1="75" x2="165" y2="135" class="divider"/>
  <line x1="330" y1="75" x2="330" y2="135" class="divider"/>

  <g>
    <text x="82" y="108" text-anchor="middle" class="stat-value">{public_repos}</text>
    <text x="82" y="125" text-anchor="middle" class="stat-label">Repositories</text>

    <text x="247" y="108" text-anchor="middle" class="stat-value">{followers}</text>
    <text x="247" y="125" text-anchor="middle" class="stat-label">Followers</text>

    <text x="412" y="108" text-anchor="middle" class="stat-value">{streak}</text>
    <text x="412" y="125" text-anchor="middle" class="stat-label">Events (30d)</text>
  </g>
  <text x="20" y="158" class="footer">↻ Updated {now}</text>
"""
...
if not top_repos:
    ...
    for i, item in enumerate(top_repos[:3]):
        ...
        # Mini Chart - adjusted to fit better
        max_weekly = max(weekly_data) if weekly_data and max(weekly_data) > 0 else 1
        chart_width, chart_height = 200, 28
        chart_x, chart_y = 260, y_pos + 2
        ...
        for j, count in enumerate(weekly_data):
            ...
            y = chart_y + chart_height - (normalized * chart_height * 0.75)
            points.append((x, y))
        ...
        repo_items.append(f"""
        <g opacity="0.95">
            <text x="20" y="{y_pos}" class="repo-name">{name}</text>
            <text x="20" y="{y_pos + 16}" class="repo-desc">{desc}</text>
            <circle cx="24" cy="{y_pos + 26}" r="4" fill="{lang_color}"/>
            <text x="33" y="{y_pos + 29}" class="repo-lang">{lang}</text>
            <text x="100" y="{y_pos + 29}" class="repo-stat">📝 {commits} commits</text>
        ...
        """)
    repos_h = y_start + len(top_repos) * 75 + 15
...
repos_body = f"""
  ...
  <text x="20" y="34" class="title">🚀 My Contributions (6 months)</text>
  {"".join(repo_items)}
"""
...
for i, day in enumerate(days_short):
    ...
    bar_height = max((count / max(max_activity, 1)) * 65, 3) if count > 0 else 0
    x_pos = 35 + i * 62
    y_base = 125
    ...
    bar_items.append(f"""
    <g>
      <rect x="{x_pos}" y="{y_base - bar_height}" width="42" height="{bar_height}" rx="5" fill="{color}" opacity="{opacity}">
      ...
      {f'<text x="{x_pos + 21}" y="{y_base - bar_height - 6}" text-anchor="middle" class="count">{count}</text>' if count > 0 else ''}
      <text x="{x_pos + 21}" y="{y_base + 16}" text-anchor="middle" class="day-label">{day}</text>
    </g>
    """)
...
weekly_body = f"""
  ...
  <text x="20" y="34" class="title">📈 Weekly Activity</text>
  <text x="20" y="52" class="subtitle">Last 7 days</text>
  <line x1="35" y1="125" x2="470" y2="125" class="baseline"/>
  {"".join(bar_items)}
"""
...
for i, (period, count) in enumerate(time_blocks.items()):
    ...
    time_items.append(f"""
    <g>
      <rect x="{x_pos}" y="70" width="62" height="65" rx="8" fill="{color}" opacity="{0.3 + intensity * 0.7}"/>
      <text x="{x_pos + 31}" y="92" text-anchor="middle" class="emoji">{emoji}</text>
      <text x="{x_pos + 31}" y="110" text-anchor="middle" class="time-count">{count}</text>
      <text x="{x_pos + 31}" y="150" text-anchor="middle" class="time-label">{lines[0]}</text>
      <text x="{x_pos + 31}" y="160" text-anchor="middle" class="time-label">{lines[1]}</text>
    </g>
    """)
...
time_body = f"""
  ...
  <text x="20" y="34" class="title">⏰ When I Code</text>
  <text x="20" y="52" class="subtitle">Last 30 days activity</text>
  {"".join(time_items)}
"""
